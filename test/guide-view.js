...
var pathData2BBox = (function () {
  ...
  function createBBox(corners) {
    return {
      ...
      right: corners.x2,
      bottom: corners.y2,
      width: corners.x2 - corners.x1,
      height: corners.y2 - corners.y1,
    };
  }
  ...
  function pathData2BBox(pathData) {
    return createBBox(
      pathData.reduce(function (corners, pathSeg) {
  ...
  function guideView() {
    ...
    Object.keys(window.insProps).forEach(function (id) {
      var props = window.insProps[id],
        ...
        guideSvg = baseDocument.body.appendChild(
          baseDocument.createElementNS(SVG_NS, "svg")
        ),
        edgeLeft,
        edgeRight;
      ...
      (function () {
        var pathSegs = [],
          curPoint;
        props.linePath.customGetPathData().forEach(function (pathSeg) {
          var values = pathSeg.values;
          if (pathSeg.type === "C") {
            pathSegs.push([
              curPoint,
      ...
      (function () {
        var path = guideSvg.appendChild(
          ...
          pathSegs = [],
          bBox = pathData2BBox(props.linePath.customGetPathData());
        [
          ["left", "top", -1, -1],
          ["right", "top", 1, -1],
          ["left", "bottom", -1, 1],
        ...
        path.customSetPathData(pathSegs);
      })();
      ...
      (function () {
        ...
        [0, 1].forEach(function (i) {
          var anchor = props.options.anchorSE[i],
            isAttach = props.optionIsAttach.anchorSE[i],
            attachProps =
              isAttach !== false ? window.insAttachProps[anchor._id] : null,
            bBox =
              isAttach !== false && attachProps.conf.getBBoxNest
                ? attachProps.conf.getBBoxNest(
                    attachProps,
                    props,
                    attachProps.conf.getStrokeWidth
      ...
      (function () {
        ...
        props.attachments.forEach(function (attachProps) {
          if (
            attachProps.conf === window.ATTACHMENTS.pathLabel &&
            (elmPath = attachProps.elmPath)
          ) {
            usePath = guideSvg.appendChild(
      ...
      (function () {
        var maskSvg = baseDocument.body.appendChild(
            baseDocument.createElementNS(SVG_NS, "svg")
          ),
          select = baseDocument.body.appendChild(
            baseDocument.createElement("select")
          ),
          elmGs = {},
          shownG;
        ...
        Array.prototype.slice
          .call(llSvg.getElementsByTagName("mask"))
          .forEach(function (mask, i) {
            var maskId = mask.id
                ? mask.id.replace(
                    new RegExp("^leader\\-line\\-" + id + "\\-"),
                    ""
                  )
                : "MASK-" + i,
            ...
            elmGs[maskId] = maskSvg.appendChild(
              baseDocument.createElementNS(SVG_NS, "g")
            );
            Array.prototype.slice
              .call(mask.childNodes)
            ...
            if (/^plug/.test(maskId)) {
              ...
              transform.setTranslate(
                maskSvg.viewBox.baseVal.x,
                maskSvg.viewBox.baseVal.y
              );
        ...
        [llSvg, maskSvg].forEach(function (svg) {
          var border = svg.appendChild(
              baseDocument.createElementNS(SVG_NS, "rect")
            ),
