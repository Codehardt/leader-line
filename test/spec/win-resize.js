...
describe("window resize", function () {
  ...
  beforeAll(function (beforeDone) {
    loadPage(
      ...
      function (frmWindow, frmDocument, body, done) {
        ...
        ll1 = new window.LeaderLine(
          document.getElementById("elm1"),
          document.getElementById("elm2")
        );
        ll2 = new window.LeaderLine(
          document.getElementById("elm3"),
          document.getElementById("elm4")
        );
        ll3 = new window.LeaderLine(
          iframeDoc.getElementById("elm1"),
          iframeDoc.getElementById("elm2")
        );
        ll4 = new window.LeaderLine(
          iframeDoc.getElementById("elm3"),
          iframeDoc.getElementById("elm4")
        );
        ...
        if (window.IS_TRIDENT) {
          // Trident fires first `resize` event when a page was loaded.
  ...
  it("update position when window is resized", function (done) {
    ...
    pathData1 = window.insProps[ll1._id].linePath.customGetPathData();
    pathData2 = window.insProps[ll2._id].linePath.customGetPathData();
    pathData3 = window.insProps[ll3._id].linePath.customGetPathData();
    pathData4 = window.insProps[ll4._id].linePath.customGetPathData();
    ...
    setTimeout(function () {
      ...
      expect(
        pathDataHasChanged(
          pathData1,
          window.insProps[ll1._id].linePath.customGetPathData()
        )
      ).toBe(true);
      expect(
        pathDataHasChanged(
          pathData2,
          window.insProps[ll2._id].linePath.customGetPathData()
        )
      ).toBe(true);
      expect(
        pathDataHasChanged(
          pathData3,
          window.insProps[ll3._id].linePath.customGetPathData()
        )
      ).toBe(true);
      expect(
        pathDataHasChanged(
          pathData4,
          window.insProps[ll4._id].linePath.customGetPathData()
        )
      ).toBe(true);
      done();
    }, 100);
  ...
  it("no-update position when sub window is resized", function (done) {
    ...
    setTimeout(function () {
      expect(traceLog.getTaggedLog("positionByWindowResize") == null).toBe(
        true
      );
      expect(
        pathDataHasChanged(
