...
module.exports = (grunt) => {
  const fs = require("fs"),
    ...
    DEST_DIR_PATH_CSS = pathUtil.join(ROOT_PATH, "leader-line.min.css"),
    PACK_LIBS = [
      ["anim", "anim.js"],
      ["pathDataPolyfill", "path-data-polyfill/path-data-polyfill.js"],
      [
        "AnimEvent",
        "../node_modules/anim-event/anim-event.min.js",
  ...
  function minCss(content) {
    return new CleanCSS({ level: { 1: { specialComments: 0 } } }).minify(
      content
    ).styles;
  }
  ...
  grunt.initConfig({
    taskHelper: {
      css: {
        options: {
          handlerByContent: () => {
            const banner = `/*! ${PKG.title || PKG.name} v${PKG.version} (c) ${
              PKG.author.name
            } ${PKG.homepage} */\n`;
            return (
              banner +
              minCss(
                fs
                  .readFileSync(CSS_PATH, { encoding: "utf8" })
                  .trim()
          ...
          handlerByContent: (content) => {
            ...
            function getCode(value) {
              ...
              return typeof value === "object"
                ? `{${Object.keys(value)
                    .map((prop) => `${prop}:${getCode(value[prop])}`)
                    .join(",")}}`
                : typeof value === "string"
            ...
            $("svg").each((i, elm) => {
              ...
              if (symbol.length && size.length && (id = symbol.attr("id"))) {
                ...
                codeSrc.SYMBOLS[id].bCircle = Math.max(
                  -bBox.left,
                  -bBox.top,
                  bBox.right,
                  bBox.bottom
                ...
                if (
                  (outlineBase = $(".outline-base", elm)).length &&
                  (outlineMax = $(".outline-max", elm)).length
                ) {
                  codeSrc.SYMBOLS[id].outlineBase =
                    parseFloat(outlineBase.attr("stroke-width")) / 2;
                  codeSrc.SYMBOLS[id].outlineMax =
                    parseFloat(outlineMax.attr("stroke-width")) /
                    2 /
                    codeSrc.SYMBOLS[id].outlineBase;
                }
            ...
            // for debug
            fs.writeFileSync(
              `${SRC_DIR_PATH}/symbols.json`,
              JSON.stringify(codeSrc.SYMBOLS)
            );

            return `var ${Object.keys(code)
              .map((codeVar) => `${codeVar}=${code[codeVar]}`)
              .join(",")};`;
          },
        },
        ...
        options: {
          handlerByContent: (content) => {
            ...
            PACK_LIBS.forEach((keyPath) => {
              code[keyPath[0]] = fs
                .readFileSync(pathUtil.join(SRC_DIR_PATH, keyPath[1]), {
                  encoding: "utf8",
                })
  ...
  grunt.registerTask("defs", ["taskHelper:getSvgDefs"]);

  grunt.registerTask("funcs", ["taskHelper:testFuncs"]);

  grunt.registerTask("build", ["defs", "taskHelper:packJs", "taskHelper:css"]);
